...
const cloudinary = require('../../config/cloudinary')
    ...
    async function editBrand(req,res) {
        try {
            ...
            const updateBrand= await Brand.findByIdAndUpdate(id,{brandName:name,description:description,brandImage:imageUrl},{new:true})

            if(updateBrand){
                return res.status(200).json({message:"Brand updated successfully"})
            }
            else{
                return res.status(400).json({error:"Brand cannot updated"})
            }
...
module.exports={loadBrandPage,loadAddbrand,addBrand,blockBrand,unBlockBrand,loadEditBrand,editBrand,dataForBrandPage}