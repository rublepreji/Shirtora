...
async function verifyOtp(req,res){
  ...
  if(referrer){
     ...
     await Wallet.updateOne({userId:referrer._id},{$inc:{balance:REFERRER_REWARD}});
     await Wallet.updateOne({userId:newUser._id},{$inc:{balance:NEW_USER_REWARD}});
...
const signup = async (req, res) => {
  try {
    const { firstName, lastName, phone, email, password, cPassword, referralCode} = req.body;
    const result= await userService.signupService(password,cPassword,email)
    if(!result.success){
      return res.render('signupPage', {message:result.message});
    }
    req.session.userOtp = result.otp;
    req.session.userData = { firstName, lastName, phone, email, hashedPassword:result.hashedPassword, referralCode};
